...
var COLS = [
  ...
  { key: 'SALDO_ATUAL', label: 'Saldo Atual', cls: 'mono', saldoAtual: true },  /* NOVO */
  { key: 'SALDO_DISP', label: 'Saldo Disp.', cls: '', saldoDisp: true },   /* NOVO */
];
...
function processData(json, fname) {
  ...
  /* Log de diagnóstico — mostra nomes exatos das colunas no console */
  if (json.length) {
    console.log('[Dashboard] Colunas detectadas na planilha:',
      Object.keys(json[0]).map(function (k) { return '"' + k + '"'; }).join(', '));
  }

  /* Normaliza chave: remove acentos, espaços extras, converte para maiúsculo */
  function normKey(k) {
    return k.trim()
  ...
  allData = json.map(function (row) {
    var norm = {};
    Object.keys(row).forEach(function (k) { norm[normKey(k)] = row[k]; });
    return {
      ...
      SITUACAO_PAI: s(norm['SITUACAO_PAI'] || norm['SITUACAO_PAI']),
      USO_PAI: s(norm['USO_PAI']),
      GRUPO_PAI: s(norm['GRUPO_PAI']),
      DATA_PRF: s(norm['DATA_PRF']),
      PREPARACAO: s(norm['PREPARACAO']),
      ...
      SITUACAO_FILHO: s(norm['SITUACAO_FILHO']),
      ANALISE: s(norm['ANALISE']),
      PERC_ENCERRADO: '',
      RECURSO_PENDENTE: s(norm['RECURSO_PENDENTE']),
      SALDO_ATUAL: n(norm['SALDO_DISPONIVEL']),   /* lê SALDO_DISPONIVEL da planilha */
      SALDO_DISP: s(norm['EM_CONDICAO']),        /* lê EM_CONDICAO da planilha */
    };
  });
  ...
  populateFilters();
  applyFilters();
  // openChangelog(); /* TEMPORARIAMENTE DESATIVADO */
}
...
function applyFilters() {
  ...
  filteredData = allData.filter(function (r) {
    ...
    /* ── Filtro Pendente — apenas rec. pendente ── */
    if (pend.size) {
    ...
    /* ── Filtro Saldo Disp. — SIM / NÃO ── */
    if (saldoD.size) {
      var saldoDispSim = String(r.SALDO_DISP).trim().toUpperCase() === 'SIM';
      if (saldoD.has('SIM') && !saldoDispSim) return false;
      if (saldoD.has('NÃO') && saldoDispSim) return false;
    }
    ...
    /* Filtro Preparação */
    if (prep.size && !prep.has(r.PREPARACAO)) return false;
    ...
    COLS.forEach(function (c) {
      ...
      /* NOVO — Saldo Disp. SIM/NÃO */
        ...
        if (val === 'SIM') {
          ...
          html += '<td class="mono" style="color:var(--muted);font-size:11px">—</td>';
        }
      }
      else {
...
function updateKPIs() {
  ...
  data.forEach(function (r) {
    if (r.OP_FILHO && r.SITUACAO_FILHO && r.SITUACAO_FILHO !== 'Enc' && !filhoMap[r.OP_FILHO])
  ...
  document.getElementById('kSub').textContent = fmtCount(data.length) + ' LINHAS TOTAIS';
  document.getElementById('kFilhos').textContent = fmtCount(Object.keys(filhosUnicos).length);
  ...
  var filhosPendentes = {};
  data.forEach(function (r) {
    if (r.OP_FILHO && r.SITUACAO_FILHO && r.SITUACAO_FILHO !== 'Enc' && r.RECURSO_PENDENTE && r.RECURSO_PENDENTE.trim() !== '')
      filhosPendentes[r.OP_FILHO] = true;
  });
  document.getElementById('kPendentes').textContent = fmtCount(Object.keys(filhosPendentes).length);
}
...
function renderModalOPsPai() {
  ...
  document.getElementById('modalBody').innerHTML = sorted.map(function (r) {
    var sdVal = String(r.SALDO_DISP).trim().toUpperCase();
    var sdCell = sdVal === 'SIM'
      ? '<span style="display:inline-flex;align-items:center;gap:4px;font-family:\'IBM Plex Mono\',monospace;font-size:11px;font-weight:700;padding:2px 8px;background:rgba(31,221,158,.1);color:var(--green);border:1px solid rgba(31,221,158,.4)">SIM</span>'
      : sdVal === 'NÃO' || sdVal === 'NAO'
        ? '<span style="display:inline-flex;align-items:center;gap:4px;font-family:\'IBM Plex Mono\',monospace;font-size:11px;font-weight:700;padding:2px 8px;background:rgba(255,77,109,.1);color:var(--red);border:1px solid rgba(255,77,109,.4)">NÃO</span>'
...
function openModalPesoFilho() {
  ...
  var rows = [];
  filteredData.forEach(function (r) {
    if (r.OP_FILHO && r.SITUACAO_FILHO && r.SITUACAO_FILHO !== 'Enc' && !filhosVisto[r.OP_FILHO]) {
  ...
  document.getElementById('modalBody').innerHTML = rows
    ...
    .map(function (r) {
      ...
      var saCor = r.SALDO_ATUAL > 0 ? 'color:var(--green)' : 'color:var(--red)';
      var sdVal = String(r.SALDO_DISP).trim().toUpperCase();
      var sdCell = sdVal === 'SIM'
        ? '<span style="font-family:\'IBM Plex Mono\',monospace;font-size:11px;font-weight:700;padding:2px 8px;background:rgba(31,221,158,.1);color:var(--green);border:1px solid rgba(31,221,158,.4)">SIM</span>'
        : sdVal === 'NÃO' || sdVal === 'NAO'
          ? '<span style="font-family:\'IBM Plex Mono\',monospace;font-size:11px;font-weight:700;padding:2px 8px;background:rgba(255,77,109,.1);color:var(--red);border:1px solid rgba(255,77,109,.4)">NÃO</span>'
          : '—';
      return '<tr>' +
        '<td class="mono op-c">' + r.OP_KEY + '</td>' +
        ...
        '<td class="mono" style="color:var(--text2);font-size:11px">' + (r.SEMANA ? 'Sem ' + r.SEMANA : '—') + '</td>' +
        '<td class="mono" style="font-size:11px;font-weight:700;' + saCor + '">' + r.SALDO_ATUAL + ' un</td>' +
        '<td>' + sdCell + '</td>' +
        '</tr>';
    }).join('');
...
function openModalOP(opKey, rows, refRow) {
  ...
  document.getElementById('modalSub').textContent =
    ...
    '  ·  Saldo Disp.: ' + (String(refRow.SALDO_DISP).trim().toUpperCase() === 'SIM' ? 'SIM' : 'NÃO');

  setModalHead(['OP FILHO', 'Produto Filho', 'Peso Filho', 'Sit. Filho', 'Recurso Pend.', 'Análise', 'Uso', '% Enc.', 'Semana', 'Saldo Atual', 'Saldo Disp.', 'Peso Pai']);
  ...
  document.getElementById('modalBody').innerHTML = rows
    ...
    .map(function (r) {
      ...
      return '<tr>' +
        ...
        '<td class="mono" style="color:' + percClr(r.PERC_ENCERRADO) + ';font-size:11px">' + (r.PERC_ENCERRADO || '—') + '</td>' +
        '<td class="mono" style="color:var(--text2);font-size:11px">' + (r.SEMANA ? 'Sem ' + r.SEMANA : '—') + '</td>' +
        '<td class="mono" style="font-size:11px;font-weight:700;' + saCor + '">' + (r.PERC_ENCERRADO === '100%' ? '—' : r.SALDO_ATUAL + ' un') + '</td>' +
        '<td>' + sdCell + '</td>' +
